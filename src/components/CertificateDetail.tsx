import Link from "next/link";
import type { Certificate } from "@/lib/types";
import { date, kg, sales, shortId } from "@/lib/data";
import { PageHead, Status } from "./Shell";
import { CopyValue } from "./CopyValue";
export function CertificateDetail({
  certificate,
  admin = false,
}: {
  certificate: Certificate;
  admin?: boolean;
}) {
  const sale = sales.find((x) => x.id === certificate.saleId);
  return (
    <>
      <PageHead
        eyebrow="Certificado de impacto"
        title={`Certificado #${shortId(certificate.id)}`}
        description="Constancia ambiental verificable emitida sobre material recuperado."
        action={<Status value={certificate.status} />}
      />
      <div className="grid detail-grid">
        <section className="card" style={{ padding: 0, overflow: "hidden" }}>
          <div className="hero-art" style={{ height: 440 }} />
          <div style={{ padding: 20 }}>
            <span className="eyebrow">Activo ambiental</span>
            <h2>{certificate.esgImpact.recycledMaterial}</h2>
            <p className="muted">
              Emitido por Livora · {date(certificate.createdAt)}
            </p>
          </div>
        </section>
        <section>
          <div className="card detail-panel">
            <span className="eyebrow">Impacto certificado</span>
            <h1>{kg(certificate.esgImpact.recycledKg)} recuperados</h1>
            <div
              className="grid"
              style={{ gridTemplateColumns: "1fr 1fr", margin: "20px 0" }}
            >
              <div className="card">
                <div className="kpi-label">CO₂ EVITADO</div>
                <div className="kpi-value" style={{ fontSize: 24 }}>
                  {kg(certificate.esgImpact.co2SavedKg)}
                </div>
              </div>
              <div className="card">
                <div className="kpi-label">AGUA AHORRADA</div>
                <div className="kpi-value" style={{ fontSize: 24 }}>
                  {certificate.esgImpact.waterSavedLiters.toLocaleString(
                    "es-PE",
                  )}{" "}
                  L
                </div>
              </div>
            </div>
            <div className="data-row">
              <span>Material</span>
              <strong>{certificate.esgImpact.recycledMaterial}</strong>
            </div>
            <div className="data-row">
              <span>Beneficiario</span>
              <strong>Coca-Cola Perú</strong>
            </div>
            <div className="data-row">
              <span>Venta de origen</span>
              <CopyValue value={certificate.saleId} prefix="#" />
            </div>
            <div className="data-row">
              <span>Centro de acopio</span>
              <strong>{sale?.center.email}</strong>
            </div>
          </div>
          <div className="card" style={{ marginTop: 16 }}>
            <div className="section-title">
              <h2>Verificación inmutable</h2>
              <span className="live">On-chain</span>
            </div>
            <div className="data-row">
              <span>IPFS</span>
              <CopyValue value={certificate.ipfsHash} />
            </div>
            <div className="data-row">
              <span>Tx hash</span>
              <CopyValue value={certificate.txHash} />
            </div>
            <div className="link-actions">
              <a
                className="btn"
                href={`https://ipfs.io/ipfs/${certificate.ipfsHash.replace("ipfs://", "")}`}
                target="_blank"
              >
                Metadata IPFS ↗
              </a>
              <a
                className="btn primary"
                href={`https://sepolia.arbiscan.io/tx/${certificate.txHash}`}
                target="_blank"
              >
                Arbiscan ↗
              </a>
            </div>
          </div>
        </section>
      </div>
      <Link
        href={admin ? "/admin/certificates" : "/company/certificates"}
        className="btn"
        style={{ display: "inline-block", marginTop: 18 }}
      >
        ← Volver a certificados
      </Link>
    </>
  );
}
