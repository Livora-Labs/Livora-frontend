  ...
  return (
    <>
      ...
      <PageHead
        ...
        description="Emisión y auditoría de constancias técnicas de mitigación asociadas a compras corporativas."
        action={
      ...
      <div className="toolbar" style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center", marginBottom: 16 }}>
        <div className="search" style={{ flex: "1 1 280px" }}>
          <input
            placeholder="Buscar empresa, material, ID o hash Stellar..."
            value={searchQuery}
            onChange={(e) => {
        ...
        >
          ...
          <option value="ACTIVE">ACTIVE (Vigente)</option>
          <option value="ISSUED">ISSUED (Emitido)</option>
          <option value="REVOKED">REVOKED (Revocado)</option>
          <option value="PENDING">PENDING (En trámite)</option>
        </select>
      </div>

      <p className="muted" style={{ fontSize: 12, marginBottom: 16 }}>
        {loading
          ? "Consultando catálogo de certificados..."
          : `Página ${page} de ${totalPages} · Mostrando ${paginatedCerts.length} de ${filteredCerts.length} certificados encontrados`}
      </p>
      ...
      <ViewStatesContainer
        ...
        onRetry={loadCerts}
        skeleton={viewMode === "grid" ? <CertsGridSkeleton /> : <TableSkeleton rows={5} columns={6} />}
        emptyTitle={certs.length === 0 ? "Sin Certificados Emitidos" : "Sin Coincidencias"}
        emptyDescription={
          certs.length === 0
            ? "No se han emitido certificados ambientales todavía en el sistema."
            : "No hay certificados que coincidan con la búsqueda o filtro aplicado."
        }
        emptyActionLabel={certs.length === 0 ? "Actualizar Lista" : "Limpiar Filtros"}
        onEmptyAction={() => {
          if (certs.length === 0) {
            loadCerts();
          } else {
        ...
        {viewMode === "grid" ? (
          <div className="grid collection-grid">
            {paginatedCerts.map((c) => (
              <CertificateCard certificate={c} key={c.id} admin />
            ))}
          </div>
        ) : (
            ...
            <table className="table" style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "var(--panel2)", borderBottom: "1px solid var(--line)", textAlign: "left" }}>
                  ...
                  <th style={{ padding: "12px 16px", fontWeight: 700 }}>CO₂ Mitigado</th>
                  <th style={{ padding: "12px 16px", fontWeight: 700 }}>Empresa Beneficiaria</th>
                  <th style={{ padding: "12px 16px", fontWeight: 700 }}>Estado</th>
                  <th style={{ padding: "12px 16px", fontWeight: 700 }}>Fecha Emisión</th>
              ...
              <tbody>
                {paginatedCerts.map((c) => {
                  ...
                  const buyerName = c.buyer?.name || c.buyer?.email || "Corporativo Livora";
                  return (
                    <tr key={c.id} style={{ borderBottom: "1px solid var(--line)" }}>
                      <td style={{ padding: "14px 16px", fontWeight: 600 }}>
                        <Link href={`/admin/certificates/${c.id}`} style={{ color: "var(--text)", textDecoration: "none" }}>
                          #{shortId(c.id)}
                        </Link>
                      </td>
                      ...
                      <td style={{ padding: "14px 16px" }}>
                        <div style={{ fontWeight: 500 }}>{buyerName}</div>
                      </td>
                      <td style={{ padding: "14px 16px" }}>
