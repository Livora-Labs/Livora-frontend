...
import { batches, date, kg, shortId } from "@/lib/data";
export default async function Detail({
  ...
  if (!b) notFound();
  const weight = Object.values(b.materialsActual || {}).reduce(
    (a, c) => a + c,
    0,
  );
  return (
      ...
      <div className="grid detail-grid">
        <section className="card" style={{ padding: 0, overflow: "hidden" }}>
          ...
          <div style={{ padding: 20 }}>
            ...
            {Object.entries(b.materialsActual || {}).map(([m, v]) => (
        ...
        <section className="detail-panel">
          <div className="card">
            ...
            <h1>
              {Object.keys(b.materialsActual || {}).join(" + ") ||
                "Lote en tránsito"}
            </h1>
            <div className="chips">
              <span className="chip">◈ {b.requests.length} recolecciones</span>
              <span className="chip">✓ Origen verificado</span>
              <span className="chip">⌁ Arbitrum Sepolia</span>
            </div>
            <div className="data-list">
              <div className="data-row">
                <span>ID del lote</span>
                <CopyValue value={b.id} />
              </div>
              <div className="data-row">
                <span>Recolector</span>
                <strong>{b.collector.email}</strong>
              </div>
              <div className="data-row">
                <span>Centro de destino</span>
                <strong>{b.destinationCenter?.email || "Pendiente"}</strong>
              </div>
              <div className="data-row">
                <span>Creado</span>
          ...
          {b.trace && (
            <div className="card" style={{ marginTop: 16 }}>
              ...
              <div className="data-row">
                <span>IPFS CID</span>
                <CopyValue value={b.trace.ipfsCid} />
              </div>
              <div className="data-row">
              ...
              <div className="link-actions">
                <a
                  className="btn"
                  href={`https://ipfs.io/ipfs/${b.trace.ipfsCid}`}
                  target="_blank"
                >
                  Ver manifiesto ↗
                </a>
                <a
                  ...
                  href={`https://sepolia.arbiscan.io/tx/${b.trace.txHash}`}
                  target="_blank"
                >
                  Ver en Arbiscan ↗
                </a>
          ...
          <div className="card" style={{ marginTop: 16 }}>
            <div className="section-title">
              ...
              <span className="muted">{b.requests.length} registros</span>
            </div>
            {b.requests.length ? (
              b.requests.map((r) => (
                <div className="network-row" key={r.id}>
                  <div>
                    ...
                    >
                      {r.latitude}, {r.longitude} · Hogar #
                      {r.householdId.slice(0, 6)}
                    </div>
                  </div>
              ...
              <p className="muted">
                Las evidencias originales pertenecen al lote consolidado.
              </p>
            )}
          </div>
        </section>
