...
export default function AdminPage() {
  ...
  const loading = loadingBatches || loadingCerts;
  const isRefreshing = refetchingBatches || refetchingCerts;
  ...
  const receivedBatches = batches.filter((b: any) => b.status === "RECEIVED");
  const transitBatches = batches.filter((b: any) => b.status === "IN_TRANSIT");
  const totalKg = receivedBatches.reduce(
    (sum: number, b: any) =>
      sum +
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 kpis" style={{ marginBottom: 24 }}>
        <Kpi
          ...
          trend="Total verificado on-chain"
          accent="var(--green)"
        />
        <Kpi
          label="LOTES TRAZADOS"
          value={`${receivedBatches.length}`}
          trend="Con evidencia blockchain"
          accent="var(--blue)"
        />
        <Kpi
          label="EN TRÁNSITO"
          value={`${transitBatches.length}`}
          trend="Camiones en ruta"
          accent="var(--amber)"
        />
        <Kpi
          label="CERTIFICADOS ESG"
          value={`${certificates.length}`}
          trend="100% auditables"
        />
      </div>
            ...
            <table className="table">
              ...
              <tbody>
                {receivedBatches.map((b: any) => {
                  ...
                  const stellarExplorerUrl = b.txHash
                    ? `https://stellar.expert/explorer/testnet/tx/${b.txHash}`
                    : null;
                  const totalBatchKg = Object.values(b.materialsActual || {}).reduce(
                    (s: number, v: any) => s + Number(v),
                    0
                  );
                      ...
                      <td style={{ fontSize: 11 }}>
                        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                          {Object.entries(b.materialsActual || {}).map(([mat, kg]: any) => (
                            <span key={mat} style={{ color: "#CBD5E1" }}>
                              {mat}: <strong style={{ color: "#10B981" }}>{Number(kg).toFixed(1)} kg</strong>
                            </span>
                          ))}
                          {totalBatchKg > 0 && (
                            <span style={{ color: "#64748B", fontSize: 10 }}>
                              Total: {totalBatchKg.toFixed(1)} kg
                            </span>
                          )}
                        </div>
                      </td>
                ...
                {certificates.map((c: any) => {
                  ...
                  return (
                    <tr key={c.id}>
                      ...
                      <td style={{ fontSize: 12, color: "#3B82F6" }}>
